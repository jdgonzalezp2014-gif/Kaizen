...
import { DRIVE_SCOPE, about, driveToken, googleClient, redirectUri } from '../_lib/gdrive.ts';
...
export const onRequestGet: PagesFunction<Env> = async ({ request, env }) => {
  ...
  const r = (await sql`SELECT google_client_id, google_client_secret_enc IS NOT NULL AS has_secret,
                              google_refresh_token_enc IS NOT NULL AS connected, google_drive_email
  ...
  const out = { ok: true, clientId: r.google_client_id, hasSecret: r.has_secret, connected: r.connected,
                email: r.google_drive_email, redirectUri: redirectUri(request),
